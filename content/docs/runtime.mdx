...
```
        pulling ── (cache hit) ──┐
            │                    │
            ▼                    ▼
        starting ─────────► failed  (with failure_reason)
            │                    ▲
            ▼                    │
        ready ◄──────────────────┤
            │ ▲                  │
...
`pulling` and `starting` are transient — the badge pulses amber in
...
- `PATH` — `<agent-root>/usr/bin` (system) or `/opt/bins` (docker,
  flat dir of symlinks → `/opt/bin-images/<name>/<name>`).
...
runtime image is free to consume an entirely different set of keys —
the [`CONFIG`](/docs/agentfile#config) directive is a runtime-extension
...
| `memory-strategy`      | `summarize` | Compactor mode — `summarize` or `sliding`.             |
...
| `temperature`          | provider    | Sampling temperature (float). `0` = deterministic.     |
| `top-p`                | provider    | Nucleus-sampling cutoff (float, `0`–`1`).              |
| `top-k`                | provider    | Top-K sampling cutoff (int).                           |
| `presence-penalty`     | provider    | Penalty for repeating tokens already present (float).  |
| `frequency-penalty`    | provider    | Penalty proportional to a token's prior frequency.     |

The five sampling knobs are forwarded to the provider only when the
key is present. Omit a key and the provider's own default applies —
`temperature: 0` means deterministic, not "use the default".
...
regardless — a custom runtime can read them via the same mechanism —
but the stock runtime logs unknown keys at debug.

See [`CONFIG`](/docs/agentfile#config) for the writing side and
[Workspaces → `agent.yaml`](/docs/workspaces#agent.yaml) for the
on-disk shape.
