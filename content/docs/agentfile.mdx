...
The spec only defines the directive's shape — a key/value pair with
...
in the [Runtime → Tunables](/docs/runtime#runtime-tunables) section
...
These are **not** CLI flags — the Agentfile (or a daemon-side
override) is the only input path.
